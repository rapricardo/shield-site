...
export const POST: APIRoute = async ({ request, cookies, redirect }) => {
  ...
  if (!productSlug || !name || !startsAt || !endsAt) {
    return redirect('/admin/turmas/nova/?erro=campos-obrigatorios');
  }
  ...
  if (error) {
    console.error('[admin/turmas/criar]', error);
    return redirect(`/admin/turmas/nova/?erro=${encodeURIComponent(error.message)}`);
  }

  return redirect('/admin/turmas/?msg=criada');
};
